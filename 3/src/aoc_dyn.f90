module aoc_dyn
    ...
    public :: resize, shrink_array, split
...
subroutine shrink_array(arr,sort,position)
  ...
  !> Splits a line into two at half
  subroutine split(inp,out1,out2)
    !> Input Line
    character(len=*), intent(in) :: inp
    !> Output Lines
    character(len=:),allocatable, intent(out) :: out1,out2

    integer :: half

    half=(len(inp)/2)
    allocate(character(len=half) :: out1)
    allocate(character(len=half) :: out2)

    out1=inp(1:half)
    out2=inp(half+1:len(inp))
